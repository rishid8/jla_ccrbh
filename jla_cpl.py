...
def E_z(z, Om, w0, wa):
    ...
    Ode = 1.0 - Om
    return np.sqrt(Om * (1.0 + z)**3 + Ode * (1.0 + z)**(3.0 * (1.0 + w0 + wa)) * np.exp(-3.0 * wa * z / (1.0 + z)))


def H_of_z(z, H0, Om, w0, wa):
    """The Hubble parameter at redshift z"""
    return H0 * E_z(z, Om, w0, wa)


def comoving_distance(z, H0, Om, w0, wa):
    """Comoving distance in Mpc for flat universe"""
    integrand = lambda zp: 1.0 / E_z(zp, Om, w0, wa)
    I, _ = integrate.quad(integrand, 0.0, z, epsabs=1e-8, epsrel=1e-8, limit=200)
    return (c_km_s / H0) * I


def luminosity_distance(z, H0, Om, w0, wa):
    return (1.0 + z) * comoving_distance(z, H0, Om, w0, wa)


def mu_theory(z, H0, Om, w0, wa):
    ...
    """
    D_L = np.array([luminosity_distance(zi, H0, Om, w0, wa) for zi in np.atleast_1d(z)])
    mu = 5.0 * np.log10(D_L) + 25.0
    return mu


def DM_rd(z, H0, Om, w0, wa, rd):
    return comoving_distance(z, H0, Om, w0, wa) / rd


def DH_rd(z, H0, Om, w0, wa, rd):
    h = H0/100.0
    return c_km_s / H_of_z(z, H0, Om, w0, wa) / rd

def DV_rd(z, H0, Om, w0, wa, rd):
    return (z * DM_rd(z, H0, Om, w0, wa, rd)**2 * DH_rd(z, H0, Om, w0, wa, rd))**(1/3)
...
def log_likelihood_BAO(H0, Om, w0, wa, rd):
    """
    ...
    val_bao = np.array([
        DV_rd(z_desi_bao_eff[0], H0, Om, w0, wa, rd),
        DM_rd(z_desi_bao_eff[1], H0, Om, w0, wa, rd), DH_rd(z_desi_bao_eff[1], H0, Om, w0, wa, rd),
        DM_rd(z_desi_bao_eff[2], H0, Om, w0, wa, rd), DH_rd(z_desi_bao_eff[2], H0, Om, w0, wa, rd),
        DM_rd(z_desi_bao_eff[3], H0, Om, w0, wa, rd), DH_rd(z_desi_bao_eff[3], H0, Om, w0, wa, rd),
        DM_rd(z_desi_bao_eff[4], H0, Om, w0, wa, rd), DH_rd(z_desi_bao_eff[4], H0, Om, w0, wa, rd),
        DM_rd(z_desi_bao_eff[5], H0, Om, w0, wa, rd), DH_rd(z_desi_bao_eff[5], H0, Om, w0, wa, rd),
        DM_rd(z_desi_bao_eff[6], H0, Om, w0, wa, rd), DH_rd(z_desi_bao_eff[6], H0, Om, w0, wa, rd)
    ])
...
def log_likelihood_SN(H0, Om, w0, wa):
    # Analytically marginalizing over M, saves time
    mu0 = mu_theory(z_dataSN, H0, Om, w0, wa)
    r = m_dataSN - mu0
...
def log_likelihood_CC(H0, Om, w0, wa):
    """
    Likelihood function for the CC data.
    """
    theory_H = np.array([H_of_z(z, H0, Om, w0, wa) for z in z_dataH])
...
def log_likelihood_total(params):
    ...
    if not (40.0 < H0 < 100.0 and 0.0 < Om < 0.5 and -2.0 < w0 < 0.0 and -1.0 < wa < 1.0 and 100.0 < rd < 200.0):
        return -np.inf # Return negative infinity for non-physical parameters

    lnL_BAO = log_likelihood_BAO(H0, Om, w0, wa, rd)
    lnL_CC = log_likelihood_CC(H0, Om, w0, wa)
    lnL_SN = log_likelihood_SN(H0, Om, w0, wa)
...
def chi2_SN(H0, Om, w0, wa):
    mu0 = mu_theory(z_dataSN, H0, Om, w0, wa)   # NOTE: this mu_theory must NOT subtract M
    r = m_dataSN - mu0
    # Compute a, b, c
    a = float(r @ (inv_cov_SN @ r))
    b = float(ones @ (inv_cov_SN @ r))
...
def chi2_CC(H0, Om, w0, wa):
    H_model = np.array([H_of_z(z, H0, Om, w0, wa) for z in z_dataH])
...
def log_prior(params):
    H0, Om, w0, wa, rd = params
    if not (40.0 < H0 < 100.0 and 0.0 < Om < 0.5 and -2.0 < w0 < 0.0 and -1.5 < wa < 1.5 and 100.0 < rd < 200.0):
        return -np.inf
...
x0 = np.array([70.0, 0.30, -0.8, -0.6, 147.0])
def neglogpost(x):
    lp = log_posterior(x)
    if not np.isfinite(lp):
        return 1e30
...
ndim = 5
p0_std = np.array([0.5, 0.02, 0.1, 0.1, 0.5])   # small spread around MAP; tweak if necessary
...
for i in range(nwalkers):
    ...
    p0[i,2] = np.clip(p0[i,2], -1.99, -0.01)
    p0[i,3] = np.clip(p0[i,3], -0.9, 0.9)
    p0[i,4] = np.clip(p0[i,4], 101.0, 199.0)
...
best = flat[best_idx]
best_H0, best_Om, best_w0, best_wa, best_rd = best
...
chi2_bao = chi2_BAO(best_H0, best_Om, best_w0, best_wa, best_rd)
chi2_cc  = chi2_CC(best_H0, best_Om, best_w0, best_wa)
chi2_sn  = chi2_SN(best_H0, best_Om, best_w0, best_wa)
...
plot_traces(chain, ['H0', 'Om', 'w0', 'wa', 'rd'], save_results_to + out_prefix + "_traces.png")

# ---------- corner plot of posterior marginalized samples ----------

#flat_log = flat.copy()
#flat_log[:, 2] = np.log(flat[:, 2])
custom_levels = [0.6827, 0.9545]
labels = [r'$H_0$', r'$\Omega_m$', r'$w_0$', r'$w_a$', r'$r_d$']
...
H_mod_plot = np.array([H_of_z(z, best_H0, best_Om, best_w0, best_wa) for z in z_plot])

plt.figure(figsize=(8,5))
plt.errorbar(z_dataH, H_obs_data, yerr=np.sqrt(np.diag(cov_H)), fmt='o', markersize=3, label='CC data')
plt.plot(z_plot, H_mod_plot, '-', label=f'model H(z) (H0={best_H0:.2f}, Om={best_Om:.3f}, w0={best_w0:.4f}, wa={best_wa:.5f})')
...
mu_mod_plot = mu_theory(z_sn_plot, best_H0, best_Om, best_w0, best_wa)
mu0_best = mu_theory(z_dataSN, best_H0, best_Om, best_w0, best_wa)
...
theory_bao_val = np.array([
    DV_rd(z_desi_bao_eff[0], best_H0, best_Om, best_w0, best_wa, best_rd),
    DM_rd(z_desi_bao_eff[1], best_H0, best_Om, best_w0, best_wa, best_rd), DH_rd(z_desi_bao_eff[1], best_H0, best_Om, best_w0, best_wa, best_rd),
    DM_rd(z_desi_bao_eff[2], best_H0, best_Om, best_w0, best_wa, best_rd), DH_rd(z_desi_bao_eff[2], best_H0, best_Om, best_w0, best_wa, best_rd),
    DM_rd(z_desi_bao_eff[3], best_H0, best_Om, best_w0, best_wa, best_rd), DH_rd(z_desi_bao_eff[3], best_H0, best_Om, best_w0, best_wa, best_rd),
    DM_rd(z_desi_bao_eff[4], best_H0, best_Om, best_w0, best_wa, best_rd), DH_rd(z_desi_bao_eff[4], best_H0, best_Om, best_w0, best_wa, best_rd),
    DM_rd(z_desi_bao_eff[5], best_H0, best_Om, best_w0, best_wa, best_rd), DH_rd(z_desi_bao_eff[5], best_H0, best_Om, best_w0, best_wa, best_rd),
    DM_rd(z_desi_bao_eff[6], best_H0, best_Om, best_w0, best_wa, best_rd), DH_rd(z_desi_bao_eff[6], best_H0, best_Om, best_w0, best_wa, best_rd)
])
...
np.savetxt(save_results_to + out_prefix + "_bestfit.txt", np.append(best, [chi2_tot, chi2_tot/ndof, AIC, BIC, chi2_bao, chi2_cc, chi2_sn]).reshape(1,-1),
           header="H0 Om w0 wa rd chi2 chi2_red AIC BIC chi2_bao chi2_cc chi2_sn")
...
for i in range(n_params):
    ...
    medians.append(p50)
    err_low.append(p50 - p16)
    err_high.append(p84 - p50)

medians = np.array(medians)
err_low = np.array(err_low)
err_high = np.array(err_high)

with open(save_results_to + out_prefix + "_median_1sigma.txt", "w") as f:
    f.write("# parameter  median  err_minus  err_plus\n")
    for name, med, lo, hi in zip(param_names, medians, err_low, err_high):
        f.write(f"{name:10s} {med:12.6f} {lo:12.6f} {hi:12.6f}\n")
