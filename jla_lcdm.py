...
def mu_theory(z, H0, Om):
    ...
    """
    D_L = np.array([luminosity_distance(zi, H0, Om) for zi in np.atleast_1d(z)])
    mu = 5.0 * np.log10(D_L) + 25.0
    return mu


def DM_rd(z, H0, Om, rd):
    return comoving_distance(z, H0, Om) / rd


def DH_rd(z, H0, Om, rd):
    h = H0/100.0
    return c_km_s / H_of_z(z, H0, Om) / rd

def DV_rd(z, H0, Om, rd):
    return (z * DM_rd(z, H0, Om, rd)**2 * DH_rd(z, H0, Om, rd))**(1/3)
...
def log_likelihood_BAO(H0, Om, rd):
    """
    ...
    val_bao = np.array([
        DV_rd(z_desi_bao_eff[0], H0, Om, rd),
        DM_rd(z_desi_bao_eff[1], H0, Om, rd), DH_rd(z_desi_bao_eff[1], H0, Om, rd),
        DM_rd(z_desi_bao_eff[2], H0, Om, rd), DH_rd(z_desi_bao_eff[2], H0, Om, rd),
        DM_rd(z_desi_bao_eff[3], H0, Om, rd), DH_rd(z_desi_bao_eff[3], H0, Om, rd),
        DM_rd(z_desi_bao_eff[4], H0, Om, rd), DH_rd(z_desi_bao_eff[4], H0, Om, rd),
        DM_rd(z_desi_bao_eff[5], H0, Om, rd), DH_rd(z_desi_bao_eff[5], H0, Om, rd),
        DM_rd(z_desi_bao_eff[6], H0, Om, rd), DH_rd(z_desi_bao_eff[6], H0, Om, rd)
    ])
...
def log_likelihood_SN(H0, Om):
    # Analytically marginalizing over M, saves time
    mu0 = mu_theory(z_dataSN, H0, Om)
    r = m_dataSN - mu0
...
def log_likelihood_CC(H0, Om):
    """
    Likelihood function for the CC data.
    """
    theory_H = np.array([H_of_z(z, H0, Om) for z in z_dataH])
...
def log_likelihood_total(params):
    ...
    if not (40.0 < H0 < 100.0 and 0.0 < Om < 0.5 and 100.0 < rd < 200.0):
        return -np.inf # Return negative infinity for non-physical parameters

    lnL_BAO = log_likelihood_BAO(H0, Om, rd)
    lnL_CC = log_likelihood_CC(H0, Om)
    lnL_SN = log_likelihood_SN(H0, Om)
...
seed = 12345
out_prefix = "LCDM"
save_results_to = "output/lcdm/"

np.random.seed(seed)

# ---------- helper chi2 functions (for reporting) ----------

def chi2_BAO(H0, Om, rd):
    val_bao = np.array([
        DV_rd(z_desi_bao_eff[0], H0, Om, rd),
        DM_rd(z_desi_bao_eff[1], H0, Om, rd), DH_rd(z_desi_bao_eff[1], H0, Om, rd),
        DM_rd(z_desi_bao_eff[2], H0, Om, rd), DH_rd(z_desi_bao_eff[2], H0, Om, rd),
...
def chi2_SN(H0, Om):
    mu0 = mu_theory(z_dataSN, H0, Om)   # NOTE: this mu_theory must NOT subtract M
    r = m_dataSN - mu0
    # Compute a, b, c
    a = float(r @ (inv_cov_SN @ r))
    b = float(ones @ (inv_cov_SN @ r))
...
def chi2_CC(H0, Om):
    H_model = np.array([H_of_z(z, H0, Om) for z in z_dataH])
...
ndim = 3
p0_std = np.array([0.5, 0.02, 0.5])   # small spread around MAP; tweak if necessary
...
for i in range(nwalkers):
    ...
    p0[i,2] = np.clip(p0[i,2], 101.0, 199.0)
...
chi2_bao = chi2_BAO(best_H0, best_Om, best_rd)
chi2_cc  = chi2_CC(best_H0, best_Om)
chi2_sn  = chi2_SN(best_H0, best_Om)
...
plot_traces(chain, ['H0', 'Om', 'rd'], save_results_to + out_prefix + "_traces.png")

# ---------- corner plot of posterior marginalized samples ----------

labels = [r'$H_0$', r'$\Omega_m$', r'$r_d$']
fig = corner.corner(flat, labels=labels, smooth=1.0, smooth1d=1.0, bins=30, hist_bin_factor=1, show_titles=False, title_fmt=".3f", plot_datapoints=False)
...
H_mod_plot = np.array([H_of_z(z, best_H0, best_Om) for z in z_plot])

plt.figure(figsize=(8,5))
plt.errorbar(z_dataH, H_obs_data, yerr=np.sqrt(np.diag(cov_H)), fmt='o', markersize=3, label='CC data')
plt.plot(z_plot, H_mod_plot, '-', label=f'model H(z) (H0={best_H0:.2f}, Om={best_Om:.3f})')
...
mu_mod_plot = mu_theory(z_sn_plot, best_H0, best_Om)
mu0_best = mu_theory(z_dataSN, best_H0, best_Om)
...
theory_bao_val = np.array([
    DV_rd(z_desi_bao_eff[0], best_H0, best_Om, best_rd),
    DM_rd(z_desi_bao_eff[1], best_H0, best_Om, best_rd), DH_rd(z_desi_bao_eff[1], best_H0, best_Om, best_rd),
    DM_rd(z_desi_bao_eff[2], best_H0, best_Om, best_rd), DH_rd(z_desi_bao_eff[2], best_H0, best_Om, best_rd),
    DM_rd(z_desi_bao_eff[3], best_H0, best_Om, best_rd), DH_rd(z_desi_bao_eff[3], best_H0, best_Om, best_rd),
    DM_rd(z_desi_bao_eff[4], best_H0, best_Om, best_rd), DH_rd(z_desi_bao_eff[4], best_H0, best_Om, best_rd),
    DM_rd(z_desi_bao_eff[5], best_H0, best_Om, best_rd), DH_rd(z_desi_bao_eff[5], best_H0, best_Om, best_rd),
    DM_rd(z_desi_bao_eff[6], best_H0, best_Om, best_rd), DH_rd(z_desi_bao_eff[6], best_H0, best_Om, best_rd)
])
...
np.savetxt(save_results_to + out_prefix + "_bestfit.txt", np.append(best, [chi2_tot, chi2_tot/ndof, AIC, BIC, chi2_bao, chi2_cc, chi2_sn]).reshape(1,-1),
           header="H0 Om rd chi2 chi2_red AIC BIC chi2_bao chi2_cc chi2_sn")
