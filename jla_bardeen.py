...
def E_z(z, Om, A):
    ...
    x = np.exp(A)
    B = (1.0 - Om)*(1.0 + x**2)**(3/2)
    return np.sqrt(Om * (1.0 + z)**3 + (B / ((1.0/(1.0 + z))**2 + x**2 )**(3/2)))
...
seed = 12345
out_prefix = "Bardeen"
save_results_to = "output/bardeen/"
...
x0 = np.array([70.0, 0.30, 0.6, 147.0])
def neglogpost(x):
    lp = log_posterior(x)
    if not np.isfinite(lp):
        return 1e30
...
labels = [r'$H_0$', r'$\Omega_m$', r'$log(Q_B)$', r'$r_d$']
