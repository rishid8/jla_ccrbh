...
def E_z(z, Om, A):
    ...
    x = np.exp(A)
    B = (1.0 - Om)*(1.0 + x**3.0)
    return np.sqrt(Om * (1.0 + z)**3 + (B / ((1.0/(1.0 + z))**3.0 + x**3.0 )))
...
seed = 12345
out_prefix = "Hayward"
save_results_to = "output/hayward/"
...
x0 = np.array([70.0, 0.30, 0.4, 147.0])
def neglogpost(x):
    lp = log_posterior(x)
    if not np.isfinite(lp):
        return 1e30
...
ndim = 4
p0_std = np.array([0.5, 0.02, 0.05, 0.5])   # small spread around MAP; tweak if necessary
...
labels = [r'$H_0$', r'$\Omega_m$', r'$log(Q_H)$', r'$r_d$']
...
np.savetxt(save_results_to + out_prefix + "_bestfit.txt", np.append(best, [chi2_tot, chi2_tot/ndof, AIC, BIC, chi2_bao, chi2_cc, chi2_sn]).reshape(1,-1),
           header="H0 Om A rd chi2 chi2_red AIC BIC chi2_bao chi2_cc chi2_sn")
