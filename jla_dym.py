...
def E_z(z, Om, A):
    x = (Om - 1.0) * A     # always negative in your priors
    u = 1.0 / x            # always negative, moderate

    brac = np.exp(u) / x   # always safe

    # LambertW: drop tiny imaginary parts, reject genuinely complex values
    W = lambertw(brac)
    if abs(W.imag) > 1e-8:
        return np.inf      # sampler rejects this point
    W = W.real
    ...
    # compute factor
    denom2 = Obh * A * (1.0 + z)**3
    arg = -1.0 / denom2    # negative, safe for exp
    factor = 1.0 - np.exp(arg)

    inside = Om*(1+z)**3 + Obh*(1+z)**3 * factor
    inside = max(inside, 1e-30)
...
def log_likelihood_total(params):
    ...
    if not (40.0 < H0 < 100.0 and 0.1 < Om < 0.5 and 0.1 < A < 1.4 and 100.0 < rd < 200.0):
...
x0 = np.array([70.0, 0.30, 1.0, 147.0])
def neglogpost(x):
    lp = log_posterior(x)
    if not np.isfinite(lp):
        return 1e30
...
ndim = 4
p0_std = np.array([0.5, 0.02, 0.01, 0.5])   # small spread around MAP; tweak if necessary
...
for i in range(nwalkers):
    ...
    p0[i,2] = np.clip(p0[i,2], 0.1, 1.399)
...
labels = [r'$H_0$', r'$\Omega_m$', r'$Q_{Dym}$', r'$r_d$']
